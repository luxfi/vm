...
type State struct {
	...
	getBlock func(context.Context, ids.ID) (vmchain.Block, error)
	// unmarshals [b] into a block
	unmarshalBlock        func(context.Context, []byte) (vmchain.Block, error)
	batchedUnmarshalBlock func(context.Context, [][]byte) ([]vmchain.Block, error)
	...
	buildBlock func(context.Context) (vmchain.Block, error)
...
type Config struct {
	...
	LastAcceptedBlock     vmchain.Block
	GetBlock              func(context.Context, ids.ID) (vmchain.Block, error)
	UnmarshalBlock        func(context.Context, []byte) (vmchain.Block, error)
	BatchedUnmarshalBlock func(context.Context, [][]byte) ([]vmchain.Block, error)
	BuildBlock            func(context.Context) (vmchain.Block, error)
...
// GetBlock returns the BlockWrapper as vmchain.Block corresponding to [blkID]
func (s *State) GetBlock(ctx context.Context, blkID ids.ID) (vmchain.Block, error) {
...
func (s *State) GetBlockInternal(ctx context.Context, blkID ids.ID) (vmchain.Block, error) {
...
func (s *State) ParseBlock(ctx context.Context, b []byte) (vmchain.Block, error) {
...
// BatchedParseBlock implements part of the vmchain.BatchedChainVM interface. In
...
// provided.
func (s *State) BatchedParseBlock(ctx context.Context, blksBytes [][]byte) ([]vmchain.Block, error) {
	blks := make([]vmchain.Block, len(blksBytes))
	...
	if s.batchedUnmarshalBlock != nil {
		...
		parsedBlks = make([]vmchain.Block, len(unparsedBlksBytes))
...
func (s *State) BuildBlockWithRuntime(ctx context.Context, rt *runtime.Runtime) (vmchain.Block, error) {
	if s.buildBlockWithRuntime == nil {
		return s.BuildBlock(ctx)
	}

	blk, err := s.buildBlockWithRuntime(ctx, rt)
	if err != nil {
		return nil, err
	}
...
func (s *State) BuildBlock(ctx context.Context) (vmchain.Block, error) {
...
func (s *State) deduplicate(blk vmchain.Block) vmchain.Block {
	blkID := blk.ID()
	// Defensive: buildBlock should not return a block that has already been verified.
	// If it does, make sure to return the existing reference to the vmchain.
...
func (s *State) addBlockOutsideConsensus(blk vmchain.Block) vmchain.Block {
...
// LastAcceptedBlockInternal returns the internal vmchain.Block that was last accepted
func (s *State) LastAcceptedBlockInternal() vmchain.Block {
