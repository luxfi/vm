...
func Serve(ctx context.Context, log log.Logger, vm block.ChainVM, opts ...grpcutils.ServerOption) error {
	...
	log.Info("chain.Serve: getting runtime address from env", "key", runtime.EngineAddressKey)
	runtimeAddr := os.Getenv(runtime.EngineAddressKey)
	if runtimeAddr == "" {
		return fmt.Errorf("required env var missing: %q", runtime.EngineAddressKey)
	}
	log.Info("chain.Serve: runtime address obtained", "addr", runtimeAddr)

	log.Info("chain.Serve: dialing runtime server", "addr", runtimeAddr)
	clientConn, err := grpcutils.Dial(runtimeAddr)
	if err != nil {
		return fmt.Errorf("failed to create client conn: %w", err)
	}
	log.Info("chain.Serve: dial succeeded, creating runtime client")

	client := gruntime.NewClient(runtimepb.NewRuntimeClient(clientConn))
	log.Info("chain.Serve: creating gRPC listener")

	listener, err := grpcutils.NewListener()
	if err != nil {
		return fmt.Errorf("failed to create new listener: %w", err)
	}
	log.Info("chain.Serve: listener created", "addr", listener.Addr().String())

	log.Info("chain.Serve: calling client.Initialize",
