...
func TestCoronaSessionInit(t *testing.T) {
	...
	for _, id := range pIDs {
		...
		signFunc := corona.Sign(cfg, pIDs[:threshold], message, workerPool)
...
// TestCoronaKeygenFullExecution tests Corona post-quantum threshold keygen
// NOTE: Corona MPC protocol rounds are under development. This test verifies
// session initialization works. Full execution test will be enabled once the
// lattice-based MPC rounds are complete.
func TestCoronaKeygenFullExecution(t *testing.T) {
	// Skip full execution test - Corona MPC rounds still under development
	t.Skip("Corona MPC keygen rounds under development - test session init only")
	...
	results, err := runTestProtocol(t, pIDs, func(id party.ID) protocol.StartFunc {
		return corona.Keygen(id, pIDs, threshold, workerPool)
	})
	require.NoError(err)
	require.Len(results, 3)
	...
	for id, result := range results {
		config, ok := result.(*corona.Config)
		require.True(ok, "result should be *corona.Config for party %s", id)
		require.NotNil(config)
	...
	t.Logf("Corona keygen completed: %d parties, threshold=%d", len(pIDs), threshold)
}

// TestCoronaSignFullExecution tests Corona keygen + sign
// NOTE: Depends on Corona keygen completing, which requires MPC rounds.
func TestCoronaSignFullExecution(t *testing.T) {
	// Skip until Corona MPC rounds are complete
	t.Skip("Corona MPC signing rounds under development")
	...
	threshold := 2
	signers := pIDs[:threshold]
	message := []byte("test message for Corona post-quantum threshold signing")
	...
	firstConfig := keygenResults[pIDs[0]].(*corona.Config)
	publicKey := firstConfig.PublicKey

	// Sign
	signResults, err := runTestProtocol(t, signers, func(id party.ID) protocol.StartFunc {
		config := keygenResults[id].(*corona.Config)
		return corona.Sign(config, signers, message, workerPool)
	})
	require.NoError(err)
	...
	for _, result := range signResults {
		...
		valid := corona.VerifySignature(publicKey, message, sigBytes)
		assert.True(t, valid, "Corona signature should verify")
		break
	}

	t.Logf("Corona sign completed: %d-of-%d post-quantum threshold signature", threshold, len(pIDs))
}
...
// TestCoronaRefreshFullExecution tests Corona share refresh protocol
// NOTE: Depends on Corona keygen completing, which requires MPC rounds.
func TestCoronaRefreshFullExecution(t *testing.T) {
	// Skip until Corona MPC rounds are complete
	t.Skip("Corona MPC refresh rounds under development")
	...
	originalPubKey := keygenResults[pIDs[0]].(*corona.Config).PublicKey

	// Refresh shares (same threshold)
	refreshResults, err := runTestProtocol(t, pIDs, func(id party.ID) protocol.StartFunc {
		config := keygenResults[id].(*corona.Config)
		return corona.Refresh(config, pIDs, threshold, workerPool)
	})
	require.NoError(err)
	...
	for id, result := range refreshResults {
		config, ok := result.(*corona.Config)
		require.True(ok, "refresh result should be *corona.Config for party %s", id)
		assert.Equal(t, originalPubKey, config.PublicKey,
			"public key should be preserved after refresh")
	}

	t.Logf("Corona refresh completed: shares updated, public key preserved")
}
